...
const findMissingRanges = (nums, lower, upper) => {
  if (!Array.isArray(nums)) throw Error('Please submit typeof Array for "nums"');
  if (typeof lower !== 'number' || typeof upper !== 'number') {
    throw Error('Please submit typeof "number" for upper &&/|| lower');
  }

  const formatRanges = (low, up) => {
    if (low === up) return String(low);
    return `${low}->${up}`;
  };

  if (nums.length === 0) {
    return [formatRanges(lower, upper)];
  }

  const returnedArray = [];

  if (lower < nums[0]) {
    returnedArray.push(formatRanges(lower, nums[0] - 1));
  }

  for (let i = 1; i < nums.length; i += 1) {
    if (nums[i] - nums[i - 1] > 1) {
      returnedArray.push(formatRanges(nums[i - 1] + 1, nums[i] - 1));
    }
  }

  if (upper > nums[nums.length - 1]) {
    returnedArray.push(formatRanges(nums[nums.length - 1] + 1, upper));
  }
...
console.log(findMissingRanges([], -3, -1)); // ["-3->-1"]
console.log(findMissingRanges([-1], -1, -1)); // []
console.log(findMissingRanges([-1], -2, -1)); // ["-2"]
