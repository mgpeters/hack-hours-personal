...
const removeDuplicates = (nums) => {
  ...
  // set a slow pointer
  let sPointer = 0;
  // loop through array of nums with slow and fast pointer
  for (let fPointer = sPointer + 1; fPointer < nums.length; fPointer += 1) {
    // check if f is !== s
    if (nums[sPointer] !== nums[fPointer]) {
      // if so, switch in place
      nums[sPointer + 1] = nums[fPointer];
      // also increment slow pointer
      sPointer += 1;
    }
  }
