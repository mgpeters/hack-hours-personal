...
const rob = (nums) => {
  // arg error checking
  if (!Array.isArray(nums)) throw Error('Please submit an array');

  let currTotal = 0;
  let priorTotal = 0;

  for (let i = 0; i < nums.length; i += 1) {
    const temp = currTotal;
    currTotal = Math.max(priorTotal + nums[i], currTotal);
    priorTotal = temp;
  }

  return Math.max(currTotal, priorTotal);
};
